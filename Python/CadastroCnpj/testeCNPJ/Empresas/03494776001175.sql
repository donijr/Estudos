...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776001175' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'RODOVIA REGIS BITTENCOURT' -- lograd
                              , '2643' -- nro
                              , '' -- cx_postal
                              , 'KM 2715 LOJA 207/208' -- compl
                              , 'CIDADE INTERCAP' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Taboao Da Serra') -- cidade_id
                              , 06768200 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Taboao Da Serra')) -- pais_id
