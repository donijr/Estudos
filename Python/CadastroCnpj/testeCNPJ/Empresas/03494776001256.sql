...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776001256' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'AMMO VAREJO' -- fantasia
                              , 'RODOVIA ANHANGUERA' -- lograd
                              , '3117' -- nro
                              , '' -- cx_postal
                              , 'KM 77.5 SETOR A' -- compl
                              , 'DISTRITO INDUSTRIAL BENEDITO STORANI' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Vinhedo') -- cidade_id
                              , 13288000 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Vinhedo')) -- pais_id
