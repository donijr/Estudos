...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776001680' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'RUA FREI GASPAR' -- lograd
                              , '365' -- nro
                              , '' -- cx_postal
                              , 'LOJA: 210;' -- compl
                              , 'CENTRO' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Sao Vicente') -- cidade_id
                              , 11310060 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Sao Vicente')) -- pais_id
