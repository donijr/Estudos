...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776002570' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'RODOVIA RAPOSO TAVARES' -- lograd
                              , 'KM23,5' -- nro
                              , '' -- cx_postal
                              , 'PAVMTO2 LUC 364' -- compl
                              , 'LAGEADINHO' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Cotia') -- cidade_id
                              , 06709015 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Cotia')) -- pais_id
