...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776002651' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'AVENIDA PIRACEMA' -- lograd
                              , '669' -- nro
                              , '' -- cx_postal
                              , 'LOJA 05 TERREO' -- compl
                              , 'TAMBORE' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Barueri') -- cidade_id
                              , 06460030 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Barueri')) -- pais_id
