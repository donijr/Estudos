...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776002813' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'AVENIDA CORONEL FERNANDO FERREIRA LEITE' -- lograd
                              , '1540' -- nro
                              , '' -- cx_postal
                              , 'LOJA 83' -- compl
                              , 'JARDIM CALIFORNIA' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Ribeirao Preto') -- cidade_id
                              , 14026900 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Ribeirao Preto')) -- pais_id
