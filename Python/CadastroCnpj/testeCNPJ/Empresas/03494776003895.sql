...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776003895' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'ALAMEDA TERRACOTA' -- lograd
                              , '545' -- nro
                              , '' -- cx_postal
                              , '' -- compl
                              , 'CERAMICA' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Sao Caetano Do Sul') -- cidade_id
                              , 09531190 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Sao Caetano Do Sul')) -- pais_id
