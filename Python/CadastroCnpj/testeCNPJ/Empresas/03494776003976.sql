...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776003976' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'SETOR SAI/SO AREA 6580 CCCV LOJA 135-K/L NIVEL TERREO' -- lograd
                              , 'SN' -- nro
                              , '' -- cx_postal
                              , '' -- compl
                              , 'GUARA' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Brasilia') -- cidade_id
                              , 71219900 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Brasilia')) -- pais_id
