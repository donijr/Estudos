...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776004000' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'AVENIDA GENERAL DAVID SARNOFF' -- lograd
                              , '5160' -- nro
                              , '' -- cx_postal
                              , 'LOJA: 52 E 53;' -- compl
                              , 'CIDADE INDUSTRIAL' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Contagem') -- cidade_id
                              , 32210110 -- cep 
                              , '1133267530' -- fone
                              , '' -- fax
                              , 'contadodres.rep@globo.com' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Contagem')) -- pais_id
