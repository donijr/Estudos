...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776004433' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'AVENIDA PAULO GRACINDO (LOT S DA GAVEA)' -- lograd
                              , '15' -- nro
                              , '' -- cx_postal
                              , 'LOJA: 09;' -- compl
                              , 'MORADA DA COLINA' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Uberlandia') -- cidade_id
                              , 38411145 -- cep 
                              , '1133267530' -- fone
                              , '' -- fax
                              , 'contadores.rep@globo.com' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Uberlandia')) -- pais_id
