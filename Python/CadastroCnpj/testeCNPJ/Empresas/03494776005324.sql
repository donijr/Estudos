...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776005324' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'RUA PADRE CARAPUCEIRO' -- lograd
                              , '777' -- nro
                              , '' -- cx_postal
                              , '278 B' -- compl
                              , 'BOA VIAGEM' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Recife') -- cidade_id
                              , 51020900 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Recife')) -- pais_id
