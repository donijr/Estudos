...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776005596' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'AVENIDA THEODORO VICTORELLI' -- lograd
                              , '150' -- nro
                              , '' -- cx_postal
                              , 'LOJA EUC L 63A ANDAR TERREO' -- compl
                              , 'INTERLAGOS' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Londrina') -- cidade_id
                              , 86027750 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Londrina')) -- pais_id
