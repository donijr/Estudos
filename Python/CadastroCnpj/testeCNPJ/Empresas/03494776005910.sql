...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776005910' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'AVENIDA ROTARY' -- lograd
                              , '624' -- nro
                              , '' -- cx_postal
                              , 'LOJA 1060 PISO L1' -- compl
                              , 'CENTRO' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Sao Bernardo Do Campo') -- cidade_id
                              , 09721000 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Sao Bernardo Do Campo')) -- pais_id
