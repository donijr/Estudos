...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776006991' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'ESTRADA JOAQUIM BUENO NETO' -- lograd
                              , '9999' -- nro
                              , '' -- cx_postal
                              , 'LOJA 04' -- compl
                              , 'RIO ABAIXO' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Itupeva') -- cidade_id
                              , 13295000 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Itupeva')) -- pais_id
