...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776007378' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'AVENIDA SELMA PARADA (BAILARINA)' -- lograd
                              , '505' -- nro
                              , '' -- cx_postal
                              , 'LOJA A-210' -- compl
                              , 'JARDIM MADALENA' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Campinas') -- cidade_id
                              , 13091605 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Campinas')) -- pais_id
