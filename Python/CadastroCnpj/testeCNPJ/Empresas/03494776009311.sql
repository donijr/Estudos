...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776009311' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ARTEX' -- fantasia
                              , 'AVENIDA GISELE CONSTANTINO' -- lograd
                              , '1850' -- nro
                              , '' -- cx_postal
                              , 'LOJA: 111;' -- compl
                              , 'PARQUE BELA VISTA' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Votorantim') -- cidade_id
                              , 18110650 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Votorantim')) -- pais_id
