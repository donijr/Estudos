...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              ...
                              , 'LOJA 130/131' -- compl
                              , 'OPERARIO' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Novo Hamburgo') -- cidade_id
                              , 93310460 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Novo Hamburgo')) -- pais_id
