...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776009826' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'ESTRADA DO COCO' -- lograd
                              , 'S/N' -- nro
                              , '' -- cx_postal
                              , 'LOJA 1030 LOJA 1031' -- compl
                              , 'ABRANTES' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Camacari') -- cidade_id
                              , 42840971 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Camacari')) -- pais_id
