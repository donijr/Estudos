...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776010409' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'RUA VISCONDE DE TAUNAY' -- lograd
                              , '235' -- nro
                              , '' -- cx_postal
                              , 'LOJA 001, 002 E 003' -- compl
                              , 'CENTRO' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Joinville') -- cidade_id
                              , 89201908 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Joinville')) -- pais_id
