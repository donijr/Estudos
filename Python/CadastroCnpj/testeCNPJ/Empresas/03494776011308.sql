...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776011308' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'RUA QUINZE DE NOVEMBRO' -- lograd
                              , '08' -- nro
                              , '' -- cx_postal
                              , 'LOJA 309A' -- compl
                              , 'CENTRO' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Niteroi') -- cidade_id
                              , 24020125 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Niteroi')) -- pais_id
