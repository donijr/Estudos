...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776012452' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'RODOVIA BR-020' -- lograd
                              , 'S/N' -- nro
                              , '' -- cx_postal
                              , ': KM 12 SETOR F; : LOJAS 11/12;' -- compl
                              , 'NOVA METROPOLE (JUREMA)' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Caucaia') -- cidade_id
                              , 61658007 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Caucaia')) -- pais_id
