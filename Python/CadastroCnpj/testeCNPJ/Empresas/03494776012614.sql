...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776012614' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'AVENIDA DEPUTADO JAMEL CECILIO' -- lograd
                              , '3300' -- nro
                              , '' -- cx_postal
                              , 'QUADRAB34 LOTE 2E LOJA S25 LOJA S29' -- compl
                              , 'JARDIM GOIAS' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Goiania') -- cidade_id
                              , 74810100 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Goiania')) -- pais_id
