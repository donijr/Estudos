...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              ...
                              , 'KM 22' -- nro
                              , '' -- cx_postal
                              , 'LOJA: 1013; LOJA: 1014;' -- compl
                              , 'ZONA RURAL' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Alexania') -- cidade_id
                              , 72930000 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Alexania')) -- pais_id
