...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776012967' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'AVENIDA TANCREDO NEVES 148' -- lograd
                              , '.' -- nro
                              , '' -- cx_postal
                              , 'LOJA 17 A 19 QUADRA T PRACA ERNESTO SIMOES FILHO 3 PAVIMENTO SHOPPING CENTER IGUATEMI' -- compl
                              , 'CAMINHO DAS ARVORES' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Salvador') -- cidade_id
                              , 41820908 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Salvador')) -- pais_id
