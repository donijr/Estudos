...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776013262' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , '' -- fantasia
                              , 'AVENIDA SANTOS DUMONT' -- lograd
                              , '5046' -- nro
                              , '' -- cx_postal
                              , 'LOTE VAR TROPICAIS Q 0001' -- compl
                              , 'PITANGUEIRAS' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Lauro De Freitas') -- cidade_id
                              , 42701260 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Lauro De Freitas')) -- pais_id
