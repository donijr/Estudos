...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776013343' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'AVENIDA PRESIDENTE KENNEDY' -- lograd
                              , '4121' -- nro
                              , '' -- cx_postal
                              , 'LOJA 2008 ANDAR PISO L2' -- compl
                              , 'AGUA VERDE' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Curitiba') -- cidade_id
                              , 80610010 -- cep 
                              , '1921002220' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Curitiba')) -- pais_id
