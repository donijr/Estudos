...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776013424' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'AVENIDA COLOMBO' -- lograd
                              , '9161' -- nro
                              , '' -- cx_postal
                              , 'LOJA 213 LOTE 1/1- A/2 E 03/A' -- compl
                              , 'GLEBA PATRIMONIO MARINGA' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Maringa') -- cidade_id
                              , 87070000 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Maringa')) -- pais_id
