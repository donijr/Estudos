...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '03494776013505' -- cod_part
                              , 'AMMO VAREJO LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'MMARTAN' -- fantasia
                              , 'AVENIDA COSTA E SILVA' -- lograd
                              , '185' -- nro
                              , '' -- cx_postal
                              , 'LOJA 214 - A' -- compl
                              , 'CR 1' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Foz Do Iguacu') -- cidade_id
                              , 85863712 -- cep 
                              , '1921022200' -- fone
                              , '' -- fax
                              , 'fiscal@ammovarejo.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Foz Do Iguacu')) -- pais_id
