...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '07663140000431' -- cod_part
                              , 'COTEMINAS S.A.' -- nome
                              , 1 -- dm_tipo_pessoa
                              , '' -- fantasia
                              , 'EST BR 101 - KM 3,5' -- lograd
                              , '3620' -- nro
                              , '' -- cx_postal
                              , '' -- compl
                              , 'DISTRITO INDUSTRIAL' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Joao Pessoa') -- cidade_id
                              , 58082040 -- cep 
                              , '8330441400' -- fone
                              , '' -- fax
                              , '' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Joao Pessoa')) -- pais_id
