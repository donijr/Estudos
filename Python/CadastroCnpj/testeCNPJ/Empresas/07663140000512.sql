...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '07663140000512' -- cod_part
                              , 'COTEMINAS S.A.' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'COTEMINAS' -- fantasia
                              , 'AVENIDA BACHAREL TOMAZ LANDIM' -- lograd
                              , 'S/N' -- nro
                              , '' -- cx_postal
                              , 'KM 2.2 BR 406' -- compl
                              , 'JARDIM LOLA' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Sao Goncalo Do Amarante') -- cidade_id
                              , 59296802 -- cep 
                              , '8440061943' -- fone
                              , '' -- fax
                              , 'gadbem@coteminas.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Sao Goncalo Do Amarante')) -- pais_id
