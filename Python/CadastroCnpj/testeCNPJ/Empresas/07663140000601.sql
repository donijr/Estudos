...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '07663140000601' -- cod_part
                              , 'COTEMINAS S.A.' -- nome
                              , 1 -- dm_tipo_pessoa
                              , '' -- fantasia
                              , 'AVENIDA DEPUTADO RAIMUNDO ASFORA' -- lograd
                              , '1001' -- nro
                              , '' -- cx_postal
                              , '' -- compl
                              , 'VELAME' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Campina Grande') -- cidade_id
                              , 58420000 -- cep 
                              , '8321024706' -- fone
                              , '' -- fax
                              , 'amilton.cunha@coteminas.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Campina Grande')) -- pais_id
