...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              ...
                              , 'KM 05 - LOTE 14' -- compl
                              , 'INDUSTRIAL' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Macaiba') -- cidade_id
                              , 59280000 -- cep 
                              , '8440061900' -- fone
                              , '' -- fax
                              , 'aurea.medeiros@coteminas.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Macaiba')) -- pais_id
