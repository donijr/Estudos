...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '17721008000140' -- cod_part
                              , 'ENCORPAR EMPREENDIMENTOS IMOBILIARIOS LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ENCORPAR EMPREENDIMENTOS IMOBILIARIOS' -- fantasia
                              , 'RUA DOS AIMORES' -- lograd
                              , '981' -- nro
                              , '' -- cx_postal
                              , 'ANDAR: 12 - PARTE;' -- compl
                              , 'FUNCIONARIOS' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Belo Horizonte') -- cidade_id
                              , 30140071 -- cep 
                              , '1121454500' -- fone
                              , '' -- fax
                              , 'marcos.moura@coteminas.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Belo Horizonte')) -- pais_id
   ...
   insert into csf_own.empresa ( id
                        ...
                        values ( vn_empresa_id -- id
                               ...
                               , 5 -- nro_tent_com_scan
                               , 5 -- nro_tent_com_dpec
                               , '/complianceServer/ComplianceFiscal/Integra/17721008000140' -- dir_integra /complianceServer/ComplianceFiscal/Integra/03965963000118/
                               , vn_multorg_id -- multorg_id
                               );
   --
