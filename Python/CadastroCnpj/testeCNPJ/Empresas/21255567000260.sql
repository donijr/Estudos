...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '21255567000260' -- cod_part
                              , 'COMPANHIA TECIDOS SANTANENSE' -- nome
                              , 1 -- dm_tipo_pessoa
                              , '' -- fantasia
                              , 'R DOUTOR ALCIDES GONCALVES' -- lograd
                              , '1500' -- nro
                              , '' -- cx_postal
                              , '' -- compl
                              , 'SANTANENSE' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Itauna') -- cidade_id
                              , 35681184 -- cep 
                              , '3732491434' -- fone
                              , '' -- fax
                              , 'aline.almeida@santanense.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Itauna')) -- pais_id
