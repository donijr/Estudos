...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '21255567001312' -- cod_part
                              , 'COMPANHIA TECIDOS SANTANENSE' -- nome
                              , 1 -- dm_tipo_pessoa
                              , '' -- fantasia
                              , 'RUA DR. JOSE LIMA GUIMARAES' -- lograd
                              , 'S/N' -- nro
                              , '' -- cx_postal
                              , '' -- compl
                              , 'SANTUARIO' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Conceicao Do Para') -- cidade_id
                              , 35668000 -- cep 
                              , '0372714077' -- fone
                              , '' -- fax
                              , '' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Conceicao Do Para')) -- pais_id
