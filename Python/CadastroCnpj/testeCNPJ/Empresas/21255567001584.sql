...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '21255567001584' -- cod_part
                              , 'COMPANHIA TECIDOS SANTANENSE' -- nome
                              , 1 -- dm_tipo_pessoa
                              , '' -- fantasia
                              , 'FAZENDA USINA DO CARIOCA' -- lograd
                              , 'S/N' -- nro
                              , '' -- cx_postal
                              , '' -- compl
                              , 'DISTRITO DO CARIOCA' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Para De Minas') -- cidade_id
                              , 35661470 -- cep 
                              , '0372310128' -- fone
                              , '' -- fax
                              , '' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Para De Minas')) -- pais_id
