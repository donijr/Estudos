...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '21255567001908' -- cod_part
                              , 'COMPANHIA TECIDOS SANTANENSE' -- nome
                              , 1 -- dm_tipo_pessoa
                              , '' -- fantasia
                              , 'FAZENDA FAZENDA DOS BRITOS' -- lograd
                              , 'SN' -- nro
                              , '' -- cx_postal
                              , '' -- compl
                              , 'ZONA RURAL' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Igaratinga') -- cidade_id
                              , 35695000 -- cep 
                              , '3732491521' -- fone
                              , '' -- fax
                              , 'jaime.veloso@santanense.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Igaratinga')) -- pais_id
