...
begin
   ...
   insert into csf_own.pessoa ( id
                       ...
                       values ( vn_pessoa_id -- id
                              , 0 -- dm_tipo_incl
                              , '22685473000102' -- cod_part
                              , 'ECONORTE EMPRESA CONSTRUTORA NORTE DE MINAS LTDA' -- nome
                              , 1 -- dm_tipo_pessoa
                              , 'ECONORTE' -- fantasia
                              , 'AVENIDA LINCOLN ALVES DOS SANTOS' -- lograd
                              , '955' -- nro
                              , '' -- cx_postal
                              , ': PARTE;' -- compl
                              , 'DISTRITO INDUSTRIAL' -- bairro
                              , (select ci.id from csf_own.cidade ci where ci.descr = 'Montes Claros') -- cidade_id
                              , 39404005 -- cep 
                              , '1121454454' -- fone
                              , '' -- fax
                              , 'marcos.moura@coteminas.com.br' -- email
                              , (select es.pais_id from csf_own.estado es where es.id = (select ci.estado_id from csf_own.cidade ci where ci.descr = 'Montes Claros')) -- pais_id
   ...
   insert into csf_own.empresa ( id
                        ...
                        values ( vn_empresa_id -- id
                               ...
                               , 5 -- nro_tent_com_scan
                               , 5 -- nro_tent_com_dpec
                               , '/complianceServer/ComplianceFiscal/Integra/22685473000102' -- dir_integra /complianceServer/ComplianceFiscal/Integra/03965963000118/
                               , vn_multorg_id -- multorg_id
                               );
   --
